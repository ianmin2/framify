...
app.run(["app","cgi","$rootScope","$location", "formlyConfig","formlyValidationMessages","$localStorage",function( app,cgi, $rootScope, $location, formlyConfig, formlyValidationMessages,$localStorage ){
   ...
   /* formlyConfig.setType({
    ...
    //! FLEXIBLY SIZED SUBMIT BUTTON
    formlyConfig.setType({
        name: "btn",
        template: '<div id="resp"></div><br><button type="submit" class="mdl-button mdl-js-button mdl-button--raised mdl-js-ripple-effect mdl-color--blue" style="min-width: 200px; margin:10px; min-height:25px; color:whitesmoke; font-weight:bold;" ng-hide="{{options.templateOptions.disabled}}">{{options.templateOptions.label}}</button>',
        defaultOptions: {
            templateOptions: {
                label: "SAVE"
            }
    ...
    formlyConfig.setType({
        name: "file-input",
        template: `<div class="formly-template-wrapper form-group" > 
